 ...
 *
 * @access public
 * @param mixed $path (default: '')
 * @param string $name 
 * @return void
 */
function thim_get_template_part( $path='', $name) {
	$template = '';
	// Look in yourtheme/name.php and yourtheme/portfolio-type/name.php
	if ( $path ) {
		$template = locate_template( array( "{$name}.php", '{$path}/' . "{$name}.php" ) );
	}
	// Get default name.php
	if ( ! $template && $name && file_exists( CORE_PLUGIN_PATH . "/portfolio-type/{$name}.php" ) ) {
		$template = CORE_PLUGIN_PATH . "/portfolio-type/{$name}.php";
	}
	// If template file doesn't exist, look in yourtheme/name.php and yourtheme/portfolio-type/name.php
	if ( ! $template ) {
		$template = locate_template( array( "{$name}.php", 'portfolio-type/' . "{$name}.php" ) );
	}
	// Allow 3rd party plugin filter template file from their plugin
	$template = apply_filters( 'thim_get_template_part', $template, $path, $name );
 ...
 * @return html
 */
function thim_related_portfolio() {
	?>
	<div class="related-portfolio col-md-12">
		<div class="module_title"><h4 class="widget-title"><?php _e( 'VIEW OTHER RELATED ITEMS', 'thimpress' ); ?></h4>
 		</div>
