...
<!--	
        Preview 
        <div class="thim-widget-preview">
			<a href="#" class="thim-widget-preview-button button-secondary"><?php _e('Preview', 'thim') ?></a>
		</div>-->
	...
	function enqueue_scripts() {
		...
		if (!wp_script_is('thim-widget-admin')) {
			...
			wp_enqueue_media();
			wp_enqueue_script('thim-widget-admin', TP_THEME_FRAMEWORK_URI . 'js/admin/widget-admin.min.js', array('jquery', 'jquery-ui-sortable', 'editor'), TP_FRAMEWORK_VERSION, true);
		...
		if (!wp_script_is('thim-widget-admin-posts-selector') && $this->using_posts_selector()) {
			...
			wp_enqueue_script('thim-widget-admin-posts-selector', plugin_dir_url(SITEORIGIN_WIDGETS_BASE_PARENT_FILE) . 'base/js/posts-selector.min.js', array('jquery', 'jquery-ui-sortable', 'jquery-ui-autocomplete', 'underscore', 'backbone'), TP_FRAMEWORK_VERSION, true);
...
//            require plugin_dir_path(__FILE__) . 'inc/color.php';
	...
	function render_field( $name, $field, $value, $repeater = array(), $is_template = false ){
		...
		if( !empty( $field['state_name'] ) ) $wrapper_classes[] = 'thim-widget-field-state-' . $field['state_name'];
		if( !empty( $field['hidden'] ) ) $wrapper_classes[] = 'thim-widget-field-is-hidden';
		if( !empty( $field['optional'] ) ) $wrapper_classes[] = 'thim-widget-field-is-optional';

		?><div class="<?php echo implode(' ', array_map('sanitize_html_class', $wrapper_classes) ) ?><?php if (!empty($field['class'])){ echo ' '.$field['class']; }?>"><?php
		...
		if( $field['type'] != 'repeater' && $field['type'] != 'checkbox' && $field['type'] != 'separator' && !empty($field['label']) ) {
			?>
			<label for="<?php echo esc_attr($field_id) ?>" class="thim-widget-field-label <?php if( empty($field['hide']) ) echo 'thim-widget-section-visible'; ?>">
				<?php
		...
		switch( $field['type'] ) {
			...
			case 'select':
				?>
				<select name="<?php echo esc_attr($this->so_get_field_name($name, $repeater)) ?>" id="<?php echo esc_attr($field_id) ?>" class="thim-widget-input thim-widget-select">
					<?php
			...
			case 'radio':
				...
				<?php foreach( $field['options'] as $k => $v ) : ?>
					<label for="<?php echo esc_attr($field_id) . '-' . $k ?>">
						<input type="radio" name="<?php echo esc_attr($this->so_get_field_name($name, $repeater)) ?>" id="<?php echo esc_attr($field_id) . '-' . $k ?>" class="siteorigin-widget-input" value="<?php echo esc_attr($k) ?>" <?php checked( $k, $value ) ?>> <?php echo esc_html($v) ?>
					</label>
				<?php endforeach; ?>
				<?php
				break;
...
//				break;
			...
			case 'widget' :
				...
				?><div class="thim-widget-section <?php if (!empty($field['hide'])) echo 'thim-widget-section-hide'; ?>"><?php
			...
			case 'section' :
				?><div class="thim-widget-section <?php if( !empty($field['hide']) ) echo 'siteorigin-widget-section-hide'; ?>"><?php
				if ( !isset($field['fields']) || empty($field['fields']) ) return;
			...
			case 'bucket' :
				...
				?><input type="text" name="<?php echo esc_attr($this->so_get_field_name($name, $repeater)) ?>" id="<?php echo esc_attr($field_id) ?>" value="<?php echo esc_attr($value) ?>" class="widefat siteorigin-widget-input" /><?php
				break;
...
function thim_widget_get_plugin_dir_path($name) {
	if (strpos($name, 'ob-') === 0)
		$name = substr($name, 4); // Handle raw widget IDs, assuming they're prefixed with ob-
 ...
 */
/*function thim_widget_get_icon_list() {
	if (empty($_GET['family']))
		exit();

	$widget_icon_families = apply_filters('thim_widgets_icon_families', array());
...
add_action('wp_ajax_thim_widgets_get_icons', 'thim_widget_get_icon_list');

function thim_widget_get_icon($icon_value, $icon_styles = false) {
	if (empty($icon_value))
		return false;
	...
	if (empty($widget_icons_enqueued[$family]) && !empty($widget_icon_families[$family]['style_uri'])) {
		wp_enqueue_style('thim-widget-icon-font-' . $family, $widget_icon_families[$family]['style_uri']);
		return '<div class="ob-icon-' . esc_attr($family) . '" data-ob-icon="' . $widget_icon_families[$family]['icons'][$icon] . '" ' . (!empty($icon_styles) ? 'style="' . implode('; ', $icon_styles) . '"' : '' ) . '></div>';
	} else {
		return false;
	}
}
