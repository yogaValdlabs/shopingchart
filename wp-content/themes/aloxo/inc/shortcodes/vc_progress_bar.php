<?php
add_shortcode( 'vc_progress_bar', 'shortcode_vc_progress_bar' );
function shortcode_vc_progress_bar( $atts, $content = null ) {
	$title = $values = $units = $bgcolor = $custombgcolor = $options = $el_class = $graphicvaluescolor = '';
	...
	$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_progress_bar wpb_content_element' . $el_class, 'vc_progress_bar' ,$atts );
	...
	foreach ( $graph_lines_data as $line ) {
		$unit = ( $units != '' ) ? ' <span class="vc_label_units" style="padding-left:' . $line['value'] . '%;color:' . $graphicvaluescolor . '">' . $line['value'] . $units . '</span>' : '';
		$output .= '<div class="wapper_bar"><small class="vc_label">' . $line['label'] . '</small>' . $unit;
